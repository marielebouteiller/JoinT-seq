...
find_rejoining_events <- function(lib, master_tlx, meta, fastq, RepSeq, keep){
  ...
  # get info about the amplicon, bait and prey
  ...
  master_unique <- read.csv(master_tlx, header=TRUE, colClasses=c("character", "integer", rep("NULL",13), "integer", rep("NULL",2), "character", rep("NULL",13)), sep="\t") %>% filter(JuncID==1)
  ds_set <- DNAStringSet(master_unique$Seq)
  names(ds_set) <- BStringSet(master_unique$Qname)
  ...
  flog.info(paste("bowtie2 parameters for prey alignemnt : ", "-f --local -D 20 -R 3 -N 0 -L 8 -i C,6 --ma 2 --mp 8,2 --np 1 --rdg 8,4 --rfg 8,4 --score-min C,40 -p 1 --no-unal --reorder -t -k 3", sep=""))

  # I remove the alignement in negative orientation i.e. V2 == 16 or 256
  # if I want to check the score :
  ...
  sam_prey <- read.csv(prey_sam_name, skip=3, sep="\t",col.names = c("Qname","Flag","", "Pos","","Cigar", rep("",14)), colClasses=c("character", "integer","NULL","integer","NULL","character", rep("NULL", 14)), header=FALSE) %>% filter(Flag != 16 & Flag != 272)
  if (!RepSeq){
    sam_prey <- sam_prey %>% group_by(Qname) %>% slice_min(Pos, with_ties=FALSE) %>% as.data.frame() # the first alignment on the sequence is kept
  }
  if (RepSeq){
    if (is.null(meta$Repeat_Start) | is.null(meta$Repeat_End)){
      flog.info("Check your metadata ! if you want to use the RepSeq argument, you need to add a Repeat_Start and a Repeat_End column !")
      return()
    }
    if (meta$Strand == "+"){
    ...
    sam_prey <- filter(sam_prey, !(Flag ==256 & Pos >= (rep_start - 1) & Pos <= (rep_end - 10) )) %>% group_by(Qname) %>% slice_min(Pos, with_ties=FALSE) %>% as.data.frame() # I remove lines with Flag==256 and Pos between 34 and 70 i.e. lines with Position in the CACA repeat which are 2ndary alignment
  }
  ds_set <- ds_set[which(names(ds_set) %in% sam_prey$Qname)]
  with_prey_fa_name <- paste(lib,"_with_prey.fa", sep="")
  writeFasta(ds_set, with_prey_fa_name, mode="w", width=20000L)
  rm(ds_set)
  ...
  flog.info(paste("bowtie2 parameters for bait alignemnt : ", "-f --local -D 20 -R 3 -N 0 -L 10 -i C,6 --ma 2 --mp 8,2 --np 1 --rdg 8,4 --rfg 8,4 --score-min C,40 -p 1 --no-unal --reorder -t ", sep=""))
  # if I want to check the score :
  ...
  sam_bait <-  read.csv(bait_sam_name, skip=3, sep="\t", col.names = c("Qname","Flag","", "Pos","","Cigar", rep("",14)), colClasses=c("character", "integer","NULL","integer","NULL","character", rep("NULL", 14)), header=FALSE) %>% arrange(Qname) %>% filter(Flag != 16 & Flag != 272)
  sam_prey <- filter(sam_prey, Qname %in% sam_bait$Qname) %>% arrange(Qname)

  # Collect the Seq and the Qlen from the master
  master_unique <- filter(master_unique, Qname %in% sam_bait$Qname) %>% arrange(Qname)

  # Create a df to collect the results, and calculate new info
  df <- data.frame(Qname=sam_prey$Qname, position_prey = sam_prey$Pos, Cigar_prey_alignement=sam_prey$Cigar, position_bait = sam_bait$Pos, Cigar_bait_alignement = sam_bait$Cigar)
  rm(sam_prey, sam_bait)
  if (nrow(df) == 0){
    flog.info("Check your metadata ! you do not have any read which align to both your bait and you prey: are the columns Chr, Start, End, Strand, Primer, 5nt_BaitEnd, 5nt_PreyStart, and Amplicon consistent ? I am sorry the script will crash :/")
  }
  df$Seq = master_unique$Seq
  df$Qlen = master_unique$Qlen
  rm(master_unique)
  ...
  if (meta$Strand =="+") { ### I do not really need these columns right now for the filter steps, so might be possible to create them later. This is not limiting right now for the memory usage, so I keep it to have the info to write in the unmod file
  ...
  flog.info("Bowtie run and info collection done, starting the filter steps")
  ...
  # I remove the "clean" "uncut or perfectly rejoined" from df to keep less variables and use less memory
  ...
  # get a list of the Qname I am interested in in the form Qname + space
  v <- sapply(df$Qname, function(x){paste(x, " ", sep = "")})
  # filter the fastq file from preprocess by keeping only reads with ID in this list
  # this function reads modifies a fastq file to get a new fastq file with ID =Qname + a space only, and keep only reads with Qname from our list
  fun <- function(x){
    # prepare new ID for this fastq sample
    l <- paste(vapply(strsplit(as.character(ShortRead::id(x))," "), `[`, 1, FUN.VALUE=character(1)), " ", sep="") # this is to split the ID, take only the first element and add a space
    l <- BStringSet(l)
    # create a new fastq with same info but different IDs containing only the Qname and a space
    seqs <- ShortRead::sread(x)
    ids <- l
    qual <- quality(x)
    new_x <- ShortReadQ(sread=seqs, quality = qual, id = ids)
    new_x[ShortRead::id(new_x) %in% v]
  }
  # Read and filter the original fastq by sample by chunk of 1M reads
  if (file.exists(paste(fastq, ".gz", sep="")) == FALSE){
    flog.info(paste("Cannot find ", fastq, ".gz, check if you have this fastq file correctly gzipped.", sep=""))
  }
  filterFastq(paste(fastq, ".gz", sep=""), "temp.fq", filter = fun, compress=FALSE, yieldSize = 1000000L)
  rm(v)
  fastq_R1 <- readFastq("temp.fq")
  
  # Use quality info to filter the reads with mismatches
  if (nrow(n10) >0){
    n10$mm1 <- NA
    n10$mm2 <- NA
    n10$mm_min <- NA
    for (l in (1:nrow(n10))){
      i <- grep(n10[l,]$Qname, ShortRead::id(fastq_R1), ignore.case = TRUE)
  ...
  rm(fastq_R1)
  flog.info("All filtering steps done")
  ...
  flog.info(paste("CHECK: total from bowtie and sum of my files are the same: ", nrow(df) == nrow(excluded) + nrow(unmod) + nrow(tlx_rejoining), sep=""))
  flog.info("Collecting all rejoining events done")
  ...
  file.remove("bait.1.bt2", "bait.2.bt2", "bait.3.bt2", "bait.4.bt2", "bait.rev.1.bt2", "bait.rev.2.bt2","prey.1.bt2", "prey.2.bt2", "prey.3.bt2", "prey.4.bt2", "prey.rev.1.bt2", "prey.rev.2.bt2", paste(lib, ".fa", sep=""), paste(lib, "_with_prey.fa", sep=""), "temp.fq")
...
qual_average <- function(n, fq){
  n$mean <- NA
  for (l in (1:nrow(n))){
    i <- grep(n[l,]$Qname, ShortRead::id(fq), ignore.case = TRUE)
...
combine <- function(tlx_rejoining, result, meta, RepSeq, lib, keep){
  flog.info(paste("Combining results from wrapper with rejoining events, RepSeq is ", RepSeq, sep=""))
  ...
  if (nrow(common_RB) == 0){
    ...
    flog.info(paste("CHECK: all Qnames are unique in the result_all file: ", nrow(result_all) == length(unique(result_all$Qname)), sep=""))
    flog.info(paste("CHECK: I have included all Qnames from result and JoinT in result_all: ", nrow(result_all) == nrow(result) + nrow(tlx_rejoining) - nrow(filter(result, Qname %in% tlx_rejoining$Qname)), sep=""))
    if (keep){
    ...
    flog.info("Keeping junctions only in main chromosomes (chr1-22) and chrX,Y,M")
    ...
    flog.info("Combining with results done")
    return()
  }

...
JoinT <- function(meta, result_folder, preprocess_folder, ...){
  whichlib <- argv$which
  ...
  for (i in lib_to_process) {
    ...
    print(paste("running JoinT for ", lib, sep=""))
    metadata <- meta_all[i,]
...
JoinT(argv$meta, argv$result, argv$preprocess, argv$which, argv$RepSeq, argv$print, argv$keep)
